...
namespace ADT {
	...
	template<typename E> class FibHeapNode
	{
		...
		FibHeapNode(FibHeapNode* _pSibling, const E& _Obj, const int64& _iKey):
	...
	private:
		...
		/// \brief Internal managment to increase allocation/deallocation and cache
		///		performance
	...
	public:
		...
		void ChangeKey(FibHeapNodeP _pElement, int64 _iNewPriority);

		/// \brief Standard operation insert
		/// \param [in] _iPriority The priority of the node.
		FibHeapNodeP Insert(const E& _Object, int64 _iPriority);
	...
	template<typename E> FibHeapNode<E>* HeapT<E>::Insert(const E& _Object, int64 _iPriority)
	{
	...
	template<typename E> void HeapT<E>::ChangeKey( FibHeapNodeP _pElement, int64 _iNewPriority )
	{
	...
	template<typename E> void HeapT<E>::Delete(FibHeapNodeP _pElement)
	{
		...
		if(_pElement != m_pRoot)
			ChangeKey(_pElement, std::numeric_limits<int64>::min());
